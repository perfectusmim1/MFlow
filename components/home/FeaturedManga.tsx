...
export default function FeaturedManga() {
  ...
  const fetchFeaturedManga = async () => {
    try {
      const response = await fetch('/api/manga?limit=7&sort=rating&order=desc');
      if (response.ok) {
        const data = await response.json();
        setManga(data.data);
      }
    } catch (error) {
      console.error('Featured manga fetch error:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <section>
        <h2 className="text-3xl font-bold mb-8 text-center">
          Öne Çıkan <span className="text-gradient">Manga & Webtoon</span>
        </h2>
